...
import { getSimilarBookRecommendations } from '@/app/actions/recommendations'
...
export default function SimilarBooksWidget({ bookId }: { bookId: string }) {
  const [recs, setRecs] = useState<RecsOutput | null>(null)
  const [isPending, startTransition] = useTransition()

  useEffect(() => {
    getSimilarBookRecommendations(bookId, false).then(setRecs)
  }, [bookId])

  function handleRefresh() {
    startTransition(async () => {
      const fresh = await getSimilarBookRecommendations(bookId, true)
      setRecs(fresh)
    })
  }
  ...
  return (
    <section className="mb-10">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">If you liked this</h2>
        <button
